...
LaBufferedLogger实现标准RubyLogger所支持的所有severity，包括`:unknown`，支持直接接收Exception对象作为message。另外支持`event(type, message, opts)`用于接收任何类型的event，以及`access`记录包含request信息的access log。
